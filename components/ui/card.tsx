...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "surface-panel group/card flex flex-col gap-5 overflow-hidden rounded-xl py-5 text-sm text-card-foreground transition-colors has-[>img:first-child]:pt-0 data-[size=sm]:gap-4 data-[size=sm]:py-4 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-xl px-5 group-data-[size=sm]/card:px-4 [.border-t]:pt-5 group-data-[size=sm]/card:[.border-t]:pt-4",
        className
      )}
      {...props}
    />
